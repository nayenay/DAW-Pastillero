...
// ... (código anterior hasta la sección isRegistroPage) ...
      ...
      if (guardarBtn) {
        guardarBtn.addEventListener("click", async () => {
          ...
          for (let i = 1; i <= dosisTotal; i++) {
              ...
              // CAMBIO AQUI: La fecha formateada es la KEY, el valor es 1 (o true)
              dosisProgramadas[finalFormattedDose] = 1; 
          ...
          const data = {
            ...
            Nota: "",
            Dosis: dosisProgramadas // El nuevo nodo con las dosis programadas (ahora con fechas como claves)
          };
      ...
      get(medRef).then(snapshot => {
        if (snapshot.exists()) {
          ...
          for (let medKey in datos) {
            ...
            if (med.Dosis) {
                // CAMBIO AQUI: Obtener las claves (que ahora son las fechas)
                const dosisKeys = Object.keys(med.Dosis); 
                let proximaDosisEncontrada = null;

                // Ordenar las claves para asegurar que las verificamos cronológicamente
                dosisKeys.sort(); // Las cadenas ISO 8601 se ordenan lexicográficamente, lo que es cronológico

                for (const doseStr of dosisKeys) { // Iterar sobre las cadenas de fecha
